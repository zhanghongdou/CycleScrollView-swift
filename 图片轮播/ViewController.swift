...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let cycleView = CycleScrollView.init(frame: CGRect(x: 0, y: 64, width: self.view.frame.width, height: 200), animationDuration: 3.0)
        ...
        for index in 0..<dataArray.count {
            let imageView = UIImageView.init(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 200))
            imageView.sd_setImage(with: URL(string: dataArray[index]))
